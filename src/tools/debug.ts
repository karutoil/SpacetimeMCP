 ...
 */
export async function stdb_debug_last_panic(
  dbName?: string,
  lines: number = 200
 ...
 * Explain an error
 */
export async function stdb_debug_explain_error(errorText: string): Promise<ToolResponse> {
  const startTime = Date.now();
 ...
 * Check server health
 */
export async function stdb_debug_server_health(dbName?: string): Promise<ToolResponse> {
  const startTime = Date.now();
