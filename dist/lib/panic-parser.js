...
export class PanicParser {
    ...
    static getSuggestions(message) {
        ...
        if (lowerMessage.includes('bigint') || lowerMessage.includes('int128')) {
            suggestions.push('SpacetimeDB uses BigInt for IDs. Use 0n as placeholder for auto-inc fields.', 'When calling reducers from client, pass BigInt values as "123n" strings.');
        }
        // Database related
        if (lowerMessage.includes('table') && lowerMessage.includes('not found')) {
            suggestions.push('Make sure the table is defined with #[spacetime(table)] attribute.', 'Run `spacetime generate` to update bindings.');
        }
        // Reducer related
        if (lowerMessage.includes('reducer')) {
            suggestions.push('Check reducer function signature matches client call.', 'Reducers cannot return data - use tables or subscriptions to get data.', 'Ensure reducer is marked as public if called externally.');
        }
        // Serialization
        if (lowerMessage.includes('serialize') || lowerMessage.includes('deserialize')) {
            suggestions.push('Check that all table fields implement required traits.', 'Use the generated type bindings for client-side serialization.');
        }
        // Connection
        if (lowerMessage.includes('connection') || lowerMessage.includes('connect')) {
            suggestions.push('Make sure SpacetimeDB server is running: `spacetime start`', 'Check the database name matches what you published.');
        }
        return suggestions;
    }
    /**
...
//# sourceMappingURL=panic-parser.js.map