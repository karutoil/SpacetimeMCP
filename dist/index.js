...
import { errorResponse } from './lib/safe-json.js';
// Import all tools
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            if (!handler) {
                return {
                    content: [{
                            type: 'text',
                            text: JSON.stringify(errorResponse('TOOL_NOT_FOUND', `Tool '${name}' not found`, { available_tools: Object.keys(tools) }, [], 0))
                        }]
                };
            }
            try {
                ...
                return {
                    content: [{
                            type: 'text',
                            text: JSON.stringify(errorResponse('TOOL_ERROR', err.message || 'Tool execution failed', { tool: name, stack: err.stack }, [], 0))
                        }]
                };
            }
        });
