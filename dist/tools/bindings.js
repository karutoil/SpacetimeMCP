 ...
 */
export async function stdb_bindings_inspect() {
    ...
    try {
        ...
        if (!bindingsPath || !fs.existsSync(bindingsPath)) {
            return errorResponse('BINDINGS_NOT_FOUND', 'Could not find module bindings', { searched: bindingsPath }, ['stdb.cli.generate_bindings', 'workspace.detect_bindings_path'], Date.now() - startTime);
        }
        ...
        return errorResponse('BINDINGS_ERROR', err.message || 'Failed to inspect bindings', null, [], Date.now() - startTime);
    }
}
/**
