 ...
 */
export async function stdb_project_describe() {
    ...
    try {
        ...
        if (!modulePath || !fs.existsSync(modulePath)) {
            return errorResponse('MODULE_NOT_FOUND', 'Could not find module source', { searched: modulePath }, ['workspace.detect_module_path'], Date.now() - startTime);
        }
        // Extract schema
        ...
        return errorResponse('DESCRIBE_ERROR', err.message || 'Failed to describe project', null, [], Date.now() - startTime);
    }
}
/**
 * Generate agent context
 */
export async function stdb_project_agent_context() {
    const startTime = Date.now();
    try {
        const projectRoot = config.projectRoot;
        ...
        return errorResponse('AGENT_CONTEXT_ERROR', err.message || 'Failed to generate agent context', null, [], Date.now() - startTime);
    }
}
