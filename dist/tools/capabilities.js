 ...
 */
export async function stdb_capabilities() {
    ...
    try {
        ...
        return errorResponse('CAPABILITIES_ERROR', err.message || 'Failed to detect capabilities', null, [], Date.now() - startTime);
    }
}
/**
 * Check if reducers can be called
 */
async function checkReducerCapability(structure, config) {
    // Need bindings
 ...
 */
async function checkServerCapability(config) {
    try {
 ...
 */
function getReducerUnavailableReason(structure, config) {
    if (!structure.bindingsPath) {
