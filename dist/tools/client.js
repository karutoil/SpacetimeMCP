 ...
 */
export async function stdb_client_connect(uri) {
    const startTime = Date.now();
    // Check permissions
    if (!config.allowWrite) {
        return errorResponse('PERMISSION_DENIED', 'Client operations require STDB_MCP_ALLOW_WRITE=1', null, [], Date.now() - startTime);
    }
    const targetUri = uri || config.uri;
    if (!targetUri) {
        return errorResponse('MISSING_URI', 'URI required. Pass as argument or set STDB_URI.', null, [], Date.now() - startTime);
    }
 ...
 */
export async function stdb_client_subscribe(subscription) {
    const startTime = Date.now();
    if (!config.allowWrite) {
        return errorResponse('PERMISSION_DENIED', 'Client operations require STDB_MCP_ALLOW_WRITE=1', null, [], Date.now() - startTime);
    }
    if (!subscription || subscription.length < 5) {
        return errorResponse('INVALID_SUBSCRIPTION', 'Valid SQL subscription required', null, [], Date.now() - startTime);
    }
    return successResponse({
        subscription,
 ...
 */
export async function stdb_client_reducer_call(params) {
    const { reducerName, args } = params;
    const startTime = Date.now();
    if (!config.allowWrite) {
        return errorResponse('PERMISSION_DENIED', 'Reducer invocation requires STDB_MCP_ALLOW_WRITE=1', null, [], Date.now() - startTime);
    }
    if (!reducerName) {
        return errorResponse('MISSING_REDUCER', 'Reducer name required', null, ['stdb_project_describe'], Date.now() - startTime);
    }
